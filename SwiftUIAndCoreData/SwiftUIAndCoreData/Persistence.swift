...
struct PersistenceController {
    ...
    func saveMovie(title: String) {
        let movie = Movie(context: self.container.viewContext)
        movie.title = title
    ...
    func getMovies() -> [Movie] {
        ...
        do {
            let movies = try self.container.viewContext.fetch(fetchRequest)
            return movies
        } catch {
            return []
        }
    ...
    init(inMemory: Bool = false) {
        container = NSPersistentContainer(name: "SwiftUIAndCoreData")
        if inMemory {
